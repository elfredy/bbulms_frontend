...
export default async function NewEducationPlanThesisPage({ params, searchParams }: Props) {
  ...
  return (
    <AdminFormPage title="Attestasiya əlavə et" hint={data.plan.name ?? planId} backHref={`/${locale}/dashboard/admin/education-plans/${planId}`}>
      <EducationPlanThesisForm locale={locale} planId={planId} lookups={lookups} defaultSemesterId={sp.semester_id ?? null} />
    </AdminFormPage>
  );
}
